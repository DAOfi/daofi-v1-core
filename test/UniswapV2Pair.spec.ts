...
describe('UniswapV2Pair', () => {
  ...
  // TODO SOON
  // it('pair: setParams', async () => {
  //   await pair.setParams(2000, 1000, 2) //slope 2, exp 1, fee .2%
  //   expect(await pair.m()).to.eq(2000)
  //   expect(await pair.n()).to.eq(1000)
  ...
  it('burn:slope 2', async () => {
    ...
    await expect(pairLinearSlope2.burn(wallet.address, overrides))
      ...
      .withArgs(pairLinearSlope2.address, wallet.address, token0Amount.sub(250))
      .to.emit(token1LinearSlope2, 'Transfer')
      .withArgs(pairLinearSlope2.address, wallet.address, token1Amount.sub(2000))
      .to.emit(pairLinearSlope2, 'Sync')
      .withArgs(250, 2000)
      .to.emit(pairLinearSlope2, 'Burn')
      .withArgs(wallet.address, token0Amount.sub(250), token1Amount.sub(2000), wallet.address)
    ...
    expect(await token0LinearSlope2.balanceOf(pairLinearSlope2.address)).to.eq(250)
    expect(await token1LinearSlope2.balanceOf(pairLinearSlope2.address)).to.eq(2000)
    const totalSupplyToken0 = await token0LinearSlope2.totalSupply()
    const totalSupplyToken1 = await token1LinearSlope2.totalSupply()
    expect(await token0LinearSlope2.balanceOf(wallet.address)).to.eq(totalSupplyToken0.sub(250))
