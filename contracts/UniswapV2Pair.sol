...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        bool feeOn = _mintFee(_reserve0, _reserve1);
        uint _totalSupply = totalSupply; // gas savings, must be defined here since totalSupply can update in _mintFee
        amount0 = liquidity.mul(token0 == baseToken ? balance0 : (m * (balance0 ** n)) / LIQUIDITY_PRECISION) / _totalSupply; // using balances ensures pro-rata distribution
        amount1 = liquidity.mul(token0 == baseToken ? balance1 : (m * (balance1 ** n)) / LIQUIDITY_PRECISION) / _totalSupply; // using balances ensures pro-rata distribution
